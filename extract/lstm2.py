...
class TrafficPredictionSystem:
    ...
    def train_model(self, epochs=100, batch_size=32, patience=10):
        ...
        try:
            ...
            # Calculate detailed metrics# In the train_model method, replace the metrics calculation with:
            ...
            metrics = {
                ...
                "r2_score": float(r2_score(y_actual, y_pred))}
    ...
    def load_trained_model(self, filename=None):
        ...
        try:
            ...
            # Use custom_object_scope to register the custom loss
            with tf.keras.utils.custom_object_scope({'custom_loss': custom_loss}):
                self.model = load_model(model_path)
            
            # Try to find corresponding scaler files
            base_filename = os.path.splitext(filename)[0]
            
            # Find scaler files with the same timestamp
            scaler_X_files = [f for f in os.listdir(MODEL_DIR) if f.startswith(f"scaler_X_{base_filename.split('_')[-1]}")]
            scaler_y_files = [f for f in os.listdir(MODEL_DIR) if f.startswith(f"scaler_y_{base_filename.split('_')[-1]}")]
            
            if scaler_X_files and scaler_y_files:
                scaler_X_path = os.path.join(MODEL_DIR, scaler_X_files[0])
                scaler_y_path = os.path.join(MODEL_DIR, scaler_y_files[0])
                
                # Load scalers
                self.scaler_X = joblib.load(scaler_X_path)
                self.scaler_y = joblib.load(scaler_y_path)
                
                print(f"Scalers loaded from {scaler_X_files[0]} and {scaler_y_files[0]}")
            else:
                print("Warning: No matching scalers found. You may need to retrain the model.")
                # Optionally, try to find the most recent scaler files
                scaler_X_files = [f for f in os.listdir(MODEL_DIR) if f.startswith("scaler_X_")]
                scaler_y_files = [f for f in os.listdir(MODEL_DIR) if f.startswith("scaler_y_")]
                
                if scaler_X_files and scaler_y_files:
                    # Sort by modification time and get the most recent
                    scaler_X_path = os.path.join(MODEL_DIR, sorted(scaler_X_files, key=lambda x: os.path.getmtime(os.path.join(MODEL_DIR, x)), reverse=True)[0])
                    scaler_y_path = os.path.join(MODEL_DIR, sorted(scaler_y_files, key=lambda x: os.path.getmtime(os.path.join(MODEL_DIR, x)), reverse=True)[0])
                    
                    self.scaler_X = joblib.load(scaler_X_path)
                    self.scaler_y = joblib.load(scaler_y_path)
                    
                    print(f"Loaded most recent scalers: {scaler_X_path} and {scaler_y_path}")
                else:
                    print("No scalers found. You must retrain the model.")
                    return False
            ...
            # If loading fails, try to rebuild the model
            try:
                print("Attempting to rebuild model architecture...")
                self.build_model()
                print("Model architecture rebuilt. Please retrain.")
    ...
    def predict_eta(self, start_detector, end_detector, current_time=None, local_weather=None):
        ...
        if start_detector not in self.speed_data['detid'].unique() or \
           end_detector not in self.speed_data['detid'].unique():
            return {"error": f"Invalid detectors: {start_detector} or {end_detector} not found"}
        
        # Ensure model and scalers are loaded
        if self.model is None:
            print("No model loaded. Attempting to load most recent model...")
            model_files = [f for f in os.listdir(MODEL_DIR) if f.endswith('.keras') or f.endswith('.h5')]
            if not model_files:
            ...
            if not self.load_trained_model(latest_model):
                return {"error": "Failed to load the model"}
        
        # Ensure scalers are properly fitted
        try:
            X_train, X_test, y_train, y_test = self.prepare_training_data()
            
            if X_train is None:
                return {"error": "Could not prepare training data for scaling"}
            
            # Refit scalers if needed
            if not hasattr(self.scaler_X, 'scale_') or not hasattr(self.scaler_y, 'scale_'):
                print("Refitting scalers...")
                self.scaler_X.fit(X_train.reshape(-1, X_train.shape[-1]))
                self.scaler_y.fit(y_train)
        
        except Exception as e:
            print(f"Scaler preparation error: {e}")
            return {"error": "Failed to prepare scalers. Retrain the model."}
        ...
        # Calculate predictions for paths
        paths_info = []
            ...
            try:
                ...
                for detector in path:
                    ...
                    if len(detector_data) < self.sequence_length:
                        print(f"Insufficient data for detector {detector}")
                        continue
                    ...
                    try:
                        ...
                        print(f"Prediction error for detector {detector}: {pred_error}")
                        continue
...
def main():
    ...
    def view_metrics(system):
        ...
        try:
            ...
            base_name = metrics_file.replace('.json', '')
            plot_files = [
                f for f in os.listdir(METRICS_DIR)
                if f.startswith(f"training_analysis_{metrics['timestamp']}")
                or f.startswith(f"correlation_matrix_{metrics['timestamp']}")
            ]
